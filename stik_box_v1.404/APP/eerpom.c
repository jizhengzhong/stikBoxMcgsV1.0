...
uint8_t ee_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
	...
	address = (uint8_t )(_usAddress>>8);    //高8位
	...
	i2c_SendByte((uint8_t)_usAddress);     //低8位
...
void I2C_EE_BufferWrite(uint8_t* pBuffer, uint16_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  if(Addr == 0) 
  {
    ...
    if(NumOfPage == 0) 
    {
      ee_WriteBytes(pBuffer, WriteAddr, NumOfSingle);
//      I2C_EE_WaitEepromStandbyState();
    ...
    else  
    {
      while(NumOfPage--)
      {
        ee_WriteBytes(pBuffer, WriteAddr, EEPROM_PAGE_SIZE); 
//    	I2C_EE_WaitEepromStandbyState();
...
//      I2C_EE_WaitEepromStandbyState();
    ...
    else
    {
      ...
      if(count != 0)
      {  
        ee_WriteBytes(pBuffer, WriteAddr, count);
//        I2C_EE_WaitEepromStandbyState();
        WriteAddr += count;
        pBuffer += count;
      } 
